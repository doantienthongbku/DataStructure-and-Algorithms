...
//    }
    ...
    int countNode(Node* subRoot) {
        if (subRoot == 0) return 0;
        return ((subRoot->pLeft) && subRoot->pRight) ? 1 + countNode(subRoot->pLeft) + countNode(subRoot->pRight) : 0;
    }
...
int main() {
    ...
    cout << binaryTree.countTwoChildrenNode() << endl;
